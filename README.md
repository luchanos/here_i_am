...
<p>
    ...
    - расскажу вам о том, что такое кластер, шардирование, репликация,
    как строятся отказоустойчивые распределенные системы хранения данных
...
<p>
    ...
    - создание очередей и exchange для организации распределения задач
    
    Типовая задача: мы должны получать и обрабатывать события от другого сервиса. 
    На python писался consumer и sender, а в RMQ создавалась очередь 
    (при необходимости exchange) в которую посылались сообщения.
